...
float StockMarket::calculateExecutionPrice(
  ...
  // use limit price of older order if both orders are limit orders
  ...
  // use limit order's price if one is market order and one is limit order
  else if (order->getLimitOrMarket() == 'L' ||
           other_order->getLimitOrMarket() == 'L') {
    if (order->getLimitOrMarket() == 'L') {
      execution_price = order->getLimitPrice();
  ...
  // use last traded price if both are market orders
  else {
...
StockMarket::executeOrders(shared_ptr<const Order> order,
  ...
  // create optional new order from remaining order
  bool remains = false;
  ...
  // adds the updated order to the orders collection if made
...
void StockMarket::processOrders(shared_ptr<const Order> order) {
  ...
  // recursively process an order if it was divided
  if (remains) {
